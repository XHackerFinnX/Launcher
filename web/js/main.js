...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const installBarPercent = document.getElementById("content-install-percent");
        ...
        worlds.forEach((w) => {
            ...
            const gamemode =
                w.gamemode != null && GAMEMODES[w.gamemode]
                    ...
                    : "";
            const meta = [
                w.version ? `<span><i class="fas fa-cube"></i> ${escapeHtml(w.version)}</span>` : "",
                gamemode ? `<span><i class="fas fa-gamepad"></i> ${escapeHtml(gamemode)}</span>` : "",
                w.hardcore ? `<span class="world-hardcore"><i class="fas fa-heart-crack"></i> Хардкор</span>` : "",
                w.size_label ? `<span><i class="fas fa-hard-drive"></i> ${escapeHtml(w.size_label)}</span>` : "",
                w.last_played ? `<span><i class="fas fa-clock"></i> ${escapeHtml(w.last_played)}</span>` : "",
            ]
                .filter(Boolean)
                .join("");
            card.innerHTML = `
