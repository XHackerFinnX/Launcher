...
function showDependencyDialog({ modName, dependencies }) {
    return new Promise((resolve) => {
        ...
        overlay.querySelector(".dep-install-all").addEventListener("click", () =>
            close(allIds),
        );
        overlay
...
function createVersionCard(version, options = {}) {
    ...
    if (type) {
        ...
        // Пользовательские сборки помечаем статусом «Модпак».
    ...
    function appendItems(items, version, loader) {
        ...
        items.forEach((m) => {
            ...
            const cats = (m.categories || [])
                .filter((c) => c !== loader)
                .slice(0, 3)
                .map((c) => `<span class="mod-card-tag">${escapeHtml(c)}</span>`)
                .join("");
            card.innerHTML = `
